      ...
      <div className="container mx-auto px-2">
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-12 gap-5 sm:gap-9  lg:gap-11 xl:gap-7 py-10 justify-between text-white">
          {links.map((link, index) => (
            <div
              key={index}
              className=" text-2xl col-span-1 md:col-span-2 lg:col-span-3 pb-3.5 sm:pb-0"
            >
              ...
              <ul className="text-base flex flex-col space-y-3">
                {link.links.map((text, index) => (
                  <li key={index} className="flex items-baseline">
                    <Link
                      to={text.link}
                      className="text-border inline-block w-full hover:text-red-600"
                    >
                      {text.name}
                    </Link>
                  </li>
          ...
          <div className="pb-3.5 sm:pb-0 col-span-1 md:col-span-2 lg:col-span-3">
            <Link to="/">
              <h2 className="varela text-2xl font-bold text-white hover:text-red-600">
                FilmFriendzy
              </h2>
            </Link>
            <p className="leading-7 text-sm text-border mt-3">
              ...
              <br />
              <span>Email: filmfriendzy@contact.com</span>
            </p>
          </div>
        </div>
      </div>
