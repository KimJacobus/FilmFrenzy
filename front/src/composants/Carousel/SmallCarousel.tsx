...
const SmallCarousel = (prop: any) => {
  ...
  useEffect(() => {
    ...
    if (prop.query.length > 15) {
      ...
      const url =
        "https://api.themoviedb.org/3/search/movie?api_key=83a1629902bd9dbacb7cf2bcff2293ab&query=" +
        prop.query;
    ...
    <div className="carousel">
      <div className="control relative">
        ...
        >
          ...
          ${
            activeItem === i
              ? "active: w-screen h-[37rem] transition-transform"
              : ""
          }`}
              onClick={() => handleClick(i)}
            >
              <h1 className="lg:hidden block text-xl text-center">
                {item.title}
              </h1>
            </div>
          ))}
